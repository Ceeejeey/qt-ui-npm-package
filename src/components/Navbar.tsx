...
interface NavLink {
  ...
  ariaLabel?: string;
  className?: string;
  isButton?: boolean; // For links styled as buttons (e.g., Help Desk)
}

interface NavbarProps {
  // Logo customization
  logo?: ReactNode; // Allow custom logo component or string
  logoText?: string; // Fallback text if no logo component
  logoHref?: string; // Logo link destination
  logoClassName?: string; // Custom logo styles
  logoAriaLabel?: string; // Accessibility label for logo

  // Navigation links
  links?: NavLink[]; // Array of link objects for nav items

  // Profile button customization
  profileIcon?: ReactNode; // Custom profile icon component
  profileAriaLabel?: string; // Accessibility label for profile button
  profileClassName?: string; // Custom profile button styles

  // Styling customization
  navClassName?: string; // Base nav styles
  containerClassName?: string; // Container div styles
  mobileMenuClassName?: string; // Mobile menu styles
  mobileToggleClassName?: string; // Mobile toggle button styles
  linkClassName?: string; // Default link styles
  linkHoverClassName?: string; // Link hover styles
  scrolledClassName?: string; // Styles when scrolled
  mobileMenuOpenClassName?: string; // Styles when mobile menu is open

  // Behavioral customization
  scrollThreshold?: number; // Pixels to scroll before applying scrolled styles
  mobileBreakpoint?: string; // Tailwind breakpoint for mobile (e.g., 'lg', 'md')
  onClick?: () => void; // Handle nav click
}
    ...
    >
      <div className={containerClassName}>
        <div className="flex items-center justify-between h-16">
          ...
          {/* Desktop Links */}
          <div
            className={`hidden ${mobileBreakpoint}:flex items-center space-x-8`}
